...
class ClickAndTweet
{
    ...
    */
    ...
    private function autoTruncate($text, $username, $url, $hashtag)
    {
        ...
        if(isset($this->settings['use_auto_truncate']) && ($this->settings['use_auto_truncate'] == 1)) {
            if($totalLength > 140) {
                $text = wp_html_excerpt($text, $totalLength - 140);
                $y = 140 - $x;
    ...
    {
        $color = $this->settings['tweet_color'];
